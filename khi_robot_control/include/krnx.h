...
#define KRNX_CYC_KRNX2AS_KIND_SUPPORTED ((1 << KRNX_CYC_KRNX2AS_KIND_SIZE) - 1)
    ...
    typedef struct
    {
        ...
        short motor_lamp;
        short cycle_lamp;
        short repeat_lamp; // ティーチ/リピートモード
        short run_lamp;    // RUN/HOLDモード
        short trigger_lamp;
        short teach_lock_lamp; // ティーチロック
        short emergency;       // 非常停止
    } TKrnxPanelInfo;
...
DECLSPEC_IMPORT int WINAPI krnx_LoadEx(int cont_no, const char *filename, FLoadCallBack cbfp, void *cb_param);
    ...
    /**
     * @brief ティーチペンダント画面情報を取得する
     *
     * @param cont_no
     * @param robot_no
     * @param panelinfo 画面情報
     * @return DECLSPEC_IMPORT リターンコード
     */
